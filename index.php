...
if ($act !== '') {
	...
	if ($act === 'list') {
		$path = isset($_GET['path']) ? $_GET['path'] : '';
		$items = file_list($path);
		if ($items === false) {
			echo json_encode(array('ok' => false, 'err' => '无效路径'));
		} else {
			echo json_encode(array('ok' => true, 'path' => $path, 'items' => $items));
		}
		exit;
	}
	...
	if ($act === 'mkdir') {
		...
		if (file_mkdir($path)) {
			...
			echo json_encode(array('ok' => false, 'err' => '创建失败'));
		}
		exit;
	}
	...
	if ($act === 'del') {
		...
		if (file_del($path)) {
			...
			echo json_encode(array('ok' => false, 'err' => '删除失败'));
		}
		exit;
	}
	...
	if ($act === 'ren') {
		...
		if (file_ren($path, $name)) {
			...
			echo json_encode(array('ok' => false, 'err' => '重命名失败'));
		}
		exit;
	}
	...
	if ($act === 'paste') {
		...
		if ($src !== '') {
			...
			echo json_encode($result ? array('ok' => true, 'name' => $result) : array('ok' => false, 'err' => '粘贴失败'));
		} else {
			// 旧方式：从 session 读剪贴板
			...
			if ($result) {
				...
				echo json_encode(array('ok' => true, 'name' => $result));
			} else {
				echo json_encode(array('ok' => false, 'err' => '粘贴失败'));
			}
		}
		exit;
	}
	...
	if ($act === 'info') {
		$path = isset($_GET['path']) ? $_GET['path'] : '';
		$info = file_info($path);
		echo json_encode($info ? array('ok' => true, 'info' => $info) : array('ok' => false));
		exit;
	}
	...
	if ($act === 'log') {
		...
		echo json_encode(log_query($limit, $offset, $filter));
		exit;
	}
	...
	// 未知 act
	echo json_encode(array('ok' => false, 'err' => '未知操作'));
	exit;
}
...
<head>
	...
	<link rel="stylesheet" href="viewer1.11.6/viewer.min.css">
	<link rel="stylesheet" href="style.css">
	<script src="viewer1.11.6/viewer.min.js"></script>
</head>
<body class="fcol">
	...
	<div class="fmbar">
		...
		<button class="btn btnsm btndanger bexit ml10"><i class="fa-solid fa-sign-out"></i> 退出</button>
	</div>
