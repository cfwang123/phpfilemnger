		...
		switch (act) {
			...
			case 'del':
				if (multi) {
					...
					if (cfg.dcf || cfg.dcd) {
						F.confirm('确定要删除选中的 ' + ps.length + ' 项吗？', doDel);
					} else {
						doDel();
					}
				} else {
			...
			case 'copyziplink':
				...
				if (navigator.clipboard) {
					navigator.clipboard.writeText(zlink).then(function(){
						...
						F.copy(zlink); F.toast('ZIP链接已复制', 'ok');
					});
				} else {
					F.copy(zlink); F.toast('ZIP链接已复制', 'ok');
				}
			...
			try {
				...
				if (r.results && r.results.length) {
					...
					if (res.ok) {
						...
						task.err = res.err || '上传失败';
						F.dialog('上传失败', '<div style="text-align:center;padding:20px;color:#ef4444;"><i class="fa-solid fa-exclamation-circle" style="font-size:48px;margin-bottom:12px;display:block;"></i><div>' + F.escHtml(task.err) + '</div></div>');
					}
				} else if (r.ok) {
					task.status = 'done';
				} else {
