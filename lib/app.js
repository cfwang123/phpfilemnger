...
(function(){
	...
	FM.win.render = function(win) {
		...
		if (isDetail) {
			...
			var cols = [
				{col:'name', label:T('name'), style:'flex:1'},
				{col:'size', label:T('size'), style:'width:90px;text-align:right'},
				{col:'ext', label:T('type'), style:'width:100px'},
				{col:'time', label:T('modified_date'), style:'width:140px'},
			];
			head.innerHTML = '';
			...
			head.innerHTML += '<div class="col coldl" style="width:100px;text-align:center;">'+T('actions_col')+'</div>';
			// 列头点击排序（事件委托）
	...
	FM.preview.open = function(path) {
		if (!FM.preview.win) {
			...
			el.innerHTML =
				'<div class="pvhead"><div class="pvtitle"><i class="ico fa-solid fa-eye"></i> <span class="pvpname"></span></div><div class="pvbtns"><button class="pvbtn close" title="'+T('close')+'"><i class="fa-solid fa-times"></i></button></div></div>'
	...
	FM.log.open = function() {
		...
		F.get('log', {limit:100,offset:0}, function(r){
			...
			var html = '';
			var actMap = {login:T('log_login'),logout:T('log_logout'),upload:T('log_upload'),download:T('log_download'),del:T('log_del'),ren:T('log_ren'),mkdir:T('log_mkdir'),paste:T('log_paste'),copy:T('log_copy'),zip:T('log_zip')};
